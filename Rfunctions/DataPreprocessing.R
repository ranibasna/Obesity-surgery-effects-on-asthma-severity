# set the date variables as date format ----
mydata <- data_all %>% mutate(across(starts_with(c("study", "date", "death")) , dmy))
# processing the drug vars
mydata <- mydata %>% mutate(across(starts_with("drug"), ~replace(., stri_isempty(.), "None")))

# One hoe encoding the drug data ----

mydata_drug_oh <- mydata %>% select(id, starts_with("drug")) %>% mutate(across(where(is.character), factor)) %>% 
...
smok_data <- mydata %>% select(starts_with(c("smok","id")))
...
# replace Unknown wiith NA
smok_data_fixed <- smok_data_fixed %>% mutate(across(starts_with("smok"), ~ na_if(.,"Unknown"))) %>% distinct()

# Processing the outcome vars ----

OSC_df <- mydata %>%  select(id, date_dispensation, packages, prednisolone, date_asthma_main, date_em_asthma, date_surgery, date_first_other)
...
df_burst <- OSC_df_years %>% select(- c( y_3)) %>% dplyr::distinct()  %>% group_by(id) %>% arrange(date_dispensation, .by_group = TRUE)  %>% 
...
em_out_var <- mydata %>% select(id, date_surgery, date_em_asthma) %>% 
...
hos_out_vars <- mydata %>% select(id, date_surgery, date_asthma_main, inclusion) %>% 
...
# num data
num_data <- mydata %>% select(where(is.numeric)) %>% select(-c(prednisolone, packages)) %>% distinct()

# Define the char_data as the id plus the variiables wiith type charecter withou the smoking and the duplicate generated by the date vars
char_data <- mydata %>% select(where(is.character) | id) %>%  distinct() %>% select(-c(starts_with("smok")))
# replace the empty string with NA
char_data <- char_data %>% mutate(across(everything(), ~ na_if(.,"")))
char_data


mydata_ready <- OSC_df_years_df %>%  inner_join(df_burst_agg, by=c("id", "date_surgery")) %>% 
                inner_join(em_out_var_agg, by=c("id", "date_surgery")) %>% 
                inner_join(hos_out_vars_agg, by=c("id", "date_surgery")) %>% 
                inner_join(smok_data_fixed, by="id") %>% 
                inner_join(char_data, by="id") %>% 
                inner_join(num_data, by="id") %>% 
                inner_join(mydata_drug_oh, by="id")
